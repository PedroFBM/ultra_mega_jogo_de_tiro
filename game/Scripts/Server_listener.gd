...
func _ready():
	...
	if socket_udp.listen(listen_port) != OK:
		print("Serviço de servidor LAN: Erro em escutar porta: " + str(listen_port))
	else:
		print("Serviço de servidor LAN: Escutando porta: " + str(listen_port))
...
func clean_up():
	...
	for server_ip in known_servers:
		...
		if (now - serverInfo.lastSeen) > server_cleanup_threshold:
			...
			print('Remover servidor antigo: %s' % server_ip)
